...
class ASMGrnn(object):
    ...
    def __init__(self, hyperparams, emb_ls_dict, mlp_ls_dict, init_h_dict, train_config=None):
        ...
        for period in range(train_config['seq_length']):
            ...
            i_emb = tf.nn.embedding_lookup(item_emb_w_ys[..., period], self.input_dict[period]['i'])  # [B, H]
            h_emb = tf.nn.embedding_lookup(item_emb_w_ys[..., period], self.input_dict[period]['hist_i'])  # [B, T, H]
            u_hist = average_pooling(h_emb, self.input_dict[period]['hist_len'])  # [B, H]
            # -- emb end -------
