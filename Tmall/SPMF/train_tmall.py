...
train_config = {'method': 'SPMF_by_date',
...
def compute_prob_and_gen_set_and_update_reservoir():
    ...
    sampled_train_set = sampled_train_set.sort_values(['date']).reset_index(drop=True)

    # update pos reservoir
    t = len(data_df[(data_df['date'] < train_config['cur_date']) & (data_df['label'] == 1)])
    ...
    t = len(data_df[(data_df['date'] < train_config['cur_date']) & (data_df['label'] == 0)])
        ...
        for base_lr in [1e-2]:
            ...
            for i in range(train_config['train_start_date'], train_config['train_end_date']):
                ...
                if i == train_config['train_start_date']:
                    pos_pretrain_set = data_df[(data_df['date'] < train_config['train_start_date']) & (data_df['label'] == 1)]
                    ...
                    neg_pretrain_set = data_df[(data_df['date'] < train_config['train_start_date']) & (data_df['label'] == 0)]
